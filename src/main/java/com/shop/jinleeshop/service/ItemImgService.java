...
public class ItemImgService {
    ...
    public void updateItemImg(Long itemImgId, MultipartFile itemImgFile) throws Exception {
        ...
        if(!itemImgFile.isEmpty()) {
            ...
            /*
                변경된 상품 이미지 정보를 세팅.
                상품 등록 때처럼 itemImgRepository.save() 로직을 호출하지 않는 이유는
                savedItemImg 엔티티는 현재 영속 상태이므로 데이터를 변경하는 것만으로 변경 감지 기능이 동작하여 트랜잭션이 끝날 때 updata 쿼리가 실행
                (중요한 것은 엔티티가 영속 상태여야 한다는 것)
            */
